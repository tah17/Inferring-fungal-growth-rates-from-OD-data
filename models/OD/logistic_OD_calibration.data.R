#
# Script that defines the Stan model meta data for the latent var logistic model with delay and function which creates a Stan data list.
# To be called into any of the 01_prior_pred.R 02_post_pred.R and 03_validate/
#
get_stan_data <- function(dat, run_likelihood = NULL){
  ...
  #
  # arranges data in format stan model expects (by replicate ID)
  #
  dat %>%
    arrange(ID) -> df
...
#
# arranges data in format stan model got the data (by replicate ID) and is called into any of the 01_prior_pred.R 02_post_pred.R and 03_validate/
#
...
iter <- 2000  #number of iterations the sampler will run for - for prior predictive check this needs to be 4000 with adapt_delta at 0.99
no_of_chains <- 4  #number of chains the sampler will run for
params <- c("delta_tilde", "beta", "sigma_meas", "L", "basal", "tau")  #parameters in the model - needs to be assigned as params
#
# population parameters of the model
# to be called by tidybayes::gather_draws
#
pop_params2lang <- "cbind(delta_tilde, beta, sigma_meas, L, basal, tau)"
