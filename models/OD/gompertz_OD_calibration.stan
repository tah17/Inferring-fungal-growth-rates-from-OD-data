//
// Latent gompertz function modelled with multplicative noise
//
data {
  ...
  int<lower=0> N_exp;  // number of exps (OD data that aren't blanks)
...
parameters {
  ...
  real<lower=log10(min(IC_scaled))> delta_tilde;  // linear transform parameter
  real<lower=0> beta;  // growth rate
  real<lower=0> basal;  // linear transform parameter
  real<lower=0> sigma_meas;  // scale of the noise
...
transformed parameters {
  ...
  for (i in 1:N_exp) {
    f[i] = IC_scaled[exp_idx[i]]*exp(log(K/IC_scaled[exp_idx[i]])*(1-exp(-(beta*fmax(time[exp_idx[i]]-tau, 0.0)))));  // gompertz function
  }
}
