...
transformed parameters {
  ...
  for (i in 1:N_exp) {
    f[i] = K/(1 + ((K - IC_scaled[exp_idx[i]])/IC_scaled[exp_idx[i]])*exp(-beta*time[exp_idx[i]]));  // logistic function
  }
}
