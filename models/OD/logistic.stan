//
// Logistic function with a delay in beginning growth modelled with multplicative noise 
//
data {
...
parameters {
  real<lower=0> tau;  // delay to growth
  real<lower=0> L;  // carrying capacity
  real<lower=0> basal;  // modelled mean of blanks
...
transformed parameters {
  ...
  for (i in 1:N_exp) {
    f[i] = L/(1 + ((L-f_0)/f_0)*exp(-beta*fmax(time[exp_idx[i]]-tau, 0.0)));  // logistic function where fmax implements the delay to growth
  }
}
...
model {
  ...
  if (include_likelihood) {
    y[exp_idx] ~ lognormal(log(f), sigma_meas);
...
generated quantities {
  ...
  y_tot[exp_idx] = lognormal_rng(log(f), sigma_meas);
  for (i in c_idx)
  ...
  if (include_likelihood) {
    ...
    for (j in 1:N_exp)
      log_lik[exp_idx[j]] = lognormal_lpdf(y_obs[exp_idx[j]] | log(f[j]), sigma_meas);
  }
