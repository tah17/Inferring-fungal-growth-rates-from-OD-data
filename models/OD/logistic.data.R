#
# Script that defines the Stan model meta data for the basic logistic model with growth delay and function which creates a Stan data list.
#
get_stan_data <- function(dat, run_likelihood = NULL){
  ...
  #
  # arranges data in format stan model expects data
  #
  dat %>%
    arrange(ID) -> df
...
adapt_delta <- 0.99
params <- c("f_0", "beta", "sigma_meas", "L", "basal")  #parameters in the model - needs to be assigned as params
#
# population parameters of the model
# to be called by tidybayes::gather_draws
#
pop_params2lang <- "cbind(f_0, beta, sigma_meas, L, basal)"
