#
# Script that defines the Stan model meta data for the mixed logistic model and function which creates a Stan data list.
#
get_stan_data <- function(dat, run_likelihood = NULL){
  ...
  stan_data <- list(N_obs = length(which(df$training_data)),
                    ...
                    R = if ("fold_ID" %in% colnames(df)) length(unique(df$fold_ID)) else length(unique(df$ID)),  #number of replicates changes if CV is being used
                    R_id = if ("fold_ID" %in% colnames(df)) df$fold_ID else df$ID,
...
params <- c("delta_tilde", "sigma_mu","beta", "tau", "sigma_meas", "L")  #parameters in the model - needs to be assigned as params
#
# population parameters of the model
# to be called by tidybayes::gather_draws
#
pop_params2lang <- "cbind(sigma_mu, delta_tilde, beta, tau, sigma_meas, L)"
ind_params2lang <- "cbind(basal)[rep]"  #individual level parameters of the model
