...
get_stan_data <- function(dat, run_likelihood = NULL){
  ...
  # data must match the order of the data block in the corresponding model models/microscopy/GP.stan
  #
  ...
  stan_data <- list(N_obs = length(which(df$training_data)),
                    ...
                    x_idx = df$time + 1,
                    x = unique(df$time),
                    y_obs_train = filter(df, training_data)$length,
                    y_obs_test = filter(df, testing_data)$length,
...
#
pop_params2lang <- "cbind(sigma, rho, alpha)"
