//
// GP model for hyphal length data
//
functions {
  vector gp_pred_rng(real[] x2,  // prediction times
                     vector y1,  // observed hyphal length
...
data {
  ...
  int<lower=1, upper=R_t> x_idx[N_obs+N_miss+N_pred];
  real x[R_t];
...
generated quantities {
  ...
  if (include_likelihood) {
    ...
    growth_rate = max(g_pred);   // growth rate is maximum derivative of GP
    // for (i in 1:N_pred) {
      // log_lik[y_test_idx[i]] = normal_lpdf(log(y_obs_test[i])| f_pred[i], sigma);
