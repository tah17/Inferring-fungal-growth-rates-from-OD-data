#
# Script that defines the Stan model meta data for the logistic model and function which creates a Stan data list.
#
get_stan_data <- function(dat, run_likelihood = NULL){
  #
  # takes tibble of data and returns list that will be used as the Stan data
  # data must match the order of the data block in the corresponding model models/logistic.stan
  #
  ...
  stan_data <- list(N_obs = length(which(dat$training_data)),
                    ...
                    y_obs_train = filter(dat, training_data)$nuclei,
                    y_obs_test = filter(dat, testing_data)$nuclei,
                    time = dat$time,
                    include_likelihood = run_likelihood)
  return(stan_data)
}
#
# empty function because data was not changed when fed to stan model 
#
...
params <- c("f_0", "beta", "sigma_meas", "L", "tau")  #parameters in the model - needs to be assigned as params
#
# population parameters of the model
# to be called by tidybayes::gather_draws
#
pop_params2lang <- "cbind(f_0, beta, sigma_meas, L, tau)"
