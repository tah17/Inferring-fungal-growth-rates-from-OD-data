//
// Logistic function modelled with multplicative noise 
//
data {
  ...
  vector[N_obs] y_obs_train; // input data of nuclear count
...
parameters {
  ...
  real<lower=0> f_0;  // initial nuclear count
  real<lower=0> L;
...
transformed parameters {
  ...
  for (i in 1:N) {
    f[i] = K/(1 + ((K - f_0)/f_0)*exp(-beta*fmax(time[i]-tau, 0.0)));  // fmax implements the delay
  }
}
...
model {
  ...
  L ~ normal(2, 2);
  // beta ~ cauchy(0, 1);  // for prior sensitivity analysis needs adapt_delta=0.99
  beta ~ std_normal();
